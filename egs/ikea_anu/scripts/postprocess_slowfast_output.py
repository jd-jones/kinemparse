...
def main(
    ...
    if col_format == 'standard':
        col_names = ['video_name', 'label_id', 'label_name', 'start_frame', 'end_frame']
        ...
        err_str = (
            f"Unrecognized argument col_format={col_format}; "
            f"expected one of {accepted_args}"
        )
    ...
    slowfast_labels = pd.read_csv(
        cv_file, index_col=False, header=None, keep_default_na=False, names=col_names,
        **slowfast_csv_params
    )
    seg_ids = slowfast_labels['segment_id'].to_numpy()
